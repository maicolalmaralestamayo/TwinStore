...
export const HeroBanner: React.FC<HeroBannerProps> = ({
  ...
  const bannerTitle =
    marketplaceConfig?.bannerTitle ||
    interfaz.hero.defaultTitle;
  const bannerSubtitle =
    marketplaceConfig?.bannerSubtitle ||
    interfaz.hero.defaultSubtitle;
        ...
        <div className="flex flex-col md:flex-row items-center justify-between gap-8">
          <div className="flex-1 space-y-4 text-center md:text-left">
            <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-indigo-500/20 border border-indigo-500/30 text-indigo-300 text-xs sm:text-sm font-semibold">
              ...
              {slogan || interfaz.hero.defaultSlogan}
            </div>
          ...
          <div className="w-full md:w-auto bg-white/10 backdrop-blur-md border border-white/15 p-5 sm:p-6 rounded-2xl flex flex-col items-center text-center gap-4 min-w-[240px] shrink-0">
            ...
            <div>
              <p className="text-xs text-indigo-300 font-semibold uppercase tracking-wider">
                {interfaz.hero.catalogStatsLabel}
              </p>
              <div className="flex items-baseline justify-center gap-2 mt-1">
                <span className="text-3xl font-extrabold text-white">{productsCount}</span>
                <span className="text-xs text-slate-300">{interfaz.hero.productsCountLabel}</span>
              </div>
              <p className="text-xs text-slate-400 mt-0.5">
                en <strong className="text-slate-200">{storesCount} {interfaz.hero.inStoresLabel}</strong>
              </p>
            </div>
            ...
            <button
              ...
              title={interfaz.hero.exploreOffers}
              aria-label={interfaz.hero.exploreOffers}
            ...
            >
              ...
              <span>{interfaz.hero.exploreOffers}</span>
            </button>
          </div>
        </div>

        {/* Carrusel de Tiendas y Proveedores */}
            ...
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-2">
                ...
                <h2 className="text-xs sm:text-sm font-extrabold uppercase tracking-wider text-slate-200">
                  {interfaz.hero.carouselTitle} ({activeStores.length})
                </h2>
              </div>
              ...
              <div className="flex items-center gap-1.5">
                <button
                  ...
                  title={interfaz.hero.carouselPrev}
                  aria-label={interfaz.hero.carouselPrev}
                >
                ...
                <button
                  ...
                  className="p-1.5 rounded-lg bg-white/10 hover:bg-white/20 text-slate-200 border border-white/10 transition-all cursor-pointer"
                  title={interfaz.hero.carouselNext}
                  aria-label={interfaz.hero.carouselNext}
                >
            ...
            >
              {activeStores.map((st) => {
                ...
                return (
                  <div
                    key={st.id}
                    className="bg-slate-900/60 hover:bg-slate-900/80 border border-white/15 hover:border-indigo-500/50 backdrop-blur-md rounded-2xl p-3.5 flex flex-col justify-between min-w-[240px] max-w-[260px] shrink-0 snap-start transition-all duration-200 group/card shadow-sm"
                  >
                      ...
                      <div className="flex items-start justify-between gap-2">
                        <div className="flex items-center gap-2 min-w-0">
                          ...
                          <div className="min-w-0">
                            <h3 className="font-extrabold text-sm text-white truncate group-hover/card:text-indigo-300 transition-colors">
                              {st.name}
                            </h3>
                            <span className="text-[10px] text-indigo-300 font-mono font-bold block">
                              {interfaz.productCard.ratePrefix} {formatNumberWithDots(st.usdToCupRate)} {interfaz.productCard.rateSuffix}
                            </span>
                          </div>
                        </div>
                      </div>
                      ...
                      <div className="space-y-1 text-xs text-slate-300">
                        ...
                        <div className="flex flex-wrap gap-1 text-[10px] pt-1">
                          <span className="inline-flex items-center gap-1 bg-blue-500/20 text-blue-300 border border-blue-500/30 px-1.5 py-0.5 rounded font-semibold">
                            ...
                            {st.deliveryAvailable ? interfaz.filters.options.deliveryCourier : interfaz.filters.options.deliveryPickup}
                          </span>
                          ...
                          <span className="inline-flex items-center gap-1 bg-purple-500/20 text-purple-300 border border-purple-500/30 px-1.5 py-0.5 rounded font-semibold">
                            ...
                            {st.paymentOptions?.transferAccepted ? interfaz.filters.options.payTransfer : interfaz.filters.options.payCash}
                          </span>
                        </div>
                      </div>
                    </div>
                    ...
                    <div className="pt-3 mt-2 border-t border-white/10 flex items-center gap-2">
                      <button
                        ...
                        title={`${interfaz.hero.viewStoreButton} ${st.name}`}
                        aria-label={`${interfaz.hero.viewStoreButton} ${st.name}`}
                      >
                        <ShoppingBag className="w-3 h-3 text-indigo-300" />
                        <span>{interfaz.hero.viewStoreButton}</span>
                      </button>
                      ...
                      {onOpenStoreWhatsApp && (
                        <button
                          ...
                          title={`Contactar directo a ${st.name} por WhatsApp`}
                          aria-label={`Contactar directo a ${st.name} por WhatsApp`}
                        >
