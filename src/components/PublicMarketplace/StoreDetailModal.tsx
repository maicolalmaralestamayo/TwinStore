    ...
    >
      ...
      >
        ...
        <div className="overflow-y-auto p-4 sm:p-6 space-y-5 text-slate-800 dark:text-slate-100">
          ...
          {/* Monedas y Tasas de Cambio Oficiales de la Tienda */}
          <div>
            <span className="text-[11px] font-extrabold uppercase tracking-wider text-slate-500 dark:text-slate-400 block mb-1">
              Moneda y Tasas de Cambio de la Tienda
            </span>
            <div className="bg-emerald-50 dark:bg-emerald-950/40 p-4 rounded-2xl border border-emerald-200 dark:border-emerald-800 space-y-3">
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
                <div className="flex items-center gap-2.5">
                  ...
                  <div>
                    <span className="text-xs font-bold text-slate-800 dark:text-slate-200 block">
                      Moneda Base: <span className="font-black text-emerald-800 dark:text-emerald-300 font-mono">{store.baseCurrency || 'USD'}</span>
                      {store.secondaryCurrency && (
                        <span className="text-slate-500 dark:text-slate-400 font-normal"> · Secundaria: <span className="font-bold text-slate-700 dark:text-slate-300 font-mono">{store.secondaryCurrency}</span></span>
                      )}
                    </span>
                    <span className="text-[11px] text-slate-500 dark:text-slate-400">
                      {store.secondaryCurrency
                        ? 'Precios fijados en moneda base con conversión equiparada a moneda secundaria'
                        : 'Esta tienda vende exclusivamente en su moneda base'}
                    </span>
                  </div>
                </div>
                ...
                {store.secondaryCurrency && (
                  <div className="text-right shrink-0">
                    <span className="text-sm sm:text-base font-black text-emerald-700 dark:text-emerald-400 font-mono bg-white dark:bg-slate-900 px-3 py-1.5 rounded-xl border border-emerald-300 dark:border-emerald-700 shadow-2xs">
              ...
              {/* Lista adicional de tasas si tiene más de una */}
              {store.exchangeRates && store.exchangeRates.length > 1 && (
                <div className="pt-2 border-t border-emerald-200/60 dark:border-emerald-800/60 flex flex-wrap gap-2">
                  {store.exchangeRates.map((er) => (
                    <span
                      key={er.id}
                      className="text-[11px] font-mono font-bold bg-white dark:bg-slate-900 text-emerald-800 dark:text-emerald-300 px-2 py-0.5 rounded-lg border border-emerald-300 dark:border-emerald-700"
                    >
                      1 {er.fromCurrency} = {formatNumberWithDots(er.rate)} {er.toCurrency}
                    </span>
                  ))}
                </div>
              )}
